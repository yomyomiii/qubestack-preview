...
import { SIMULATION_RESULT, RESOURCE_ESTIMATION, getSampleCode } from '../data/sampleCircuits'
import type { QuantumResult } from '../data/sampleCircuits'
...
function ResultBar({ count, maxCount, isNoise }: {
  ...
  const color = isNoise ? 'text-[#9990EB]' : 'text-[#635ADC]'
  return <span className={`min-w-0 overflow-hidden ${color}`}>{'█'.repeat(filled)}</span>
}
  ...
  return (
    <Card variant="code">
      ...
      <div className="relative">
        <pre ref={preRef} onScroll={() => setIsScrolled((preRef.current?.scrollTop ?? 0) > 0)} className="max-h-60 overflow-auto bg-white p-4 font-mono text-xs leading-relaxed text-gray-800 whitespace-pre-wrap break-words">
          {isTranspiling ? (
        ...
        <Card variant="resource">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1.5 font-mono text-xs">
            <div className="flex flex-col gap-0.5">
              ...
              <span className="font-semibold text-gray-700">{RESOURCE_ESTIMATION.qubits}</span>
            </div>
            <div className="flex flex-col gap-0.5">
              <span className="text-gray-400">Depth</span>
              <span className="font-semibold text-gray-700">{RESOURCE_ESTIMATION.depth}</span>
            </div>
            <div className="flex flex-col gap-0.5">
              <span className="text-gray-400">Gates</span>
              <span className="font-semibold text-gray-700">{RESOURCE_ESTIMATION.gates}</span>
            </div>
          </div>
        </Card>
      )}
